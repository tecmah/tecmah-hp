...
代表者の「主な経歴（抜粋）」に取引先の社名は掲載しない（`/case-studies` と同じ匿名化方針）。
`src/data/ceo-profile.ts` の `organization` は「◯◯領域の事業会社」形式の領域表記に統一し、
`id` は `career-<開始年>-<ドメイン>` 形式にする。`/case-studies` 側の id とは意図的に一致させない
（一致させると経歴と事例を突合するキーになる）。実名を出す場合は取引先ごとに個別許諾を取ること。
...
- 掲載義務期間中の公告が `MANDATORY_NOTICES` から欠けていない
  （`EXPECTED_NOTICES` から公告を消すだけでは検査対象がゼロになって素通りするため、義務を日付付きで別に持つ）
- 各公告ページが存在し、公告の記載事項・貸借対照表の全科目が「項目名 → 値」の対で一致する
  （金額を単独文字列で照合すると、同額の行が互いの検査を素通りさせてしまうため）
- 期待値自体の書き忘れもエラーにする（`mustContain` / `mustContainPairs` が空なら fail）
...
気づけない。これを埋めるため `.github/workflows/koukoku-liveness.yml` が **毎週月曜 15:17 JST** に
...
**監視自体が止まらないようにする仕掛け（重要）**

- GitHub は public リポジトリの scheduled workflow を、**リポジトリ活動が 60 日ないと自動的に無効化する**。
  無効化は静かに起こり、cron が止まっても失敗通知は出ない（＝沈黙を正常と誤解する）。
  このリポジトリには実際に 98 日・135 日の無コミット期間があったため、放置すればこの監視は
  「監視したい休眠期間」でこそ止まる。
- 対策として同ワークフローに `keepalive` ジョブを置き、最終コミットから 40 日以上経っていたら
  `.github/keepalive` を更新してコミットし、活動タイマーを延ばす。
  `GITHUB_TOKEN` による push は他のワークフローを起動しないため、`deploy.yml` は再実行されずループしない。
- cron は毎時 00 分を避けている。GitHub 公式ドキュメントが高負荷帯での遅延と
  「some queued jobs may be dropped」を明記しており、週 1 回しか機会がない監視でドロップされると次は 7 日後になるため。

**残っているリスク**: この仕組みは「GitHub Actions が動くこと」に依存している。
Actions 自体を無効化した、リポジトリを archive した、といった場合は失敗イベントすら発生しない。
掲載義務の重さに対してより強い保証が要るなら、GitHub の外側の監視サービス
（UptimeRobot 等）から `/ir/kessan/fy1` を併せて監視するのが望ましい。
ただし外形監視はキーワード 1 つしか見られないことが多く、
本スクリプトのような「貸借対照表の全科目が掲載時のままか」の検証は代替できない。併用すること。
